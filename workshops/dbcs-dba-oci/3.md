...
```
sudo su - oracle
expdp system/<password>@pdb1 directory=tmp dumpfile=source_pdb1.dmp compression=all logfile=source_pdb1_export.log full=yes encryption=all encryption_mode=password encryption_password=<password> encryption_algorithm=aes256
...
```
chmod a+r /tmp/source_pdb1.dmp
exit -- from oracle
```

- Next scp (copy) the file to the target database with the opc user and then exit from the source database.
```
scp -i ~/privateKey /tmp/source_pdb1.dmp opc@129.146.123.48:/tmp
[opc@workshop1 ~]$ scp -i ~/privateKey /tmp/source_pdb1.dmp opc@129.146.123.48:/tmp
source_pdb1.dmp                               100% 2024KB  96.6MB/s   00:00 

exit -- from opc source database
```
...
```
create pluggable database pdb2 admin user alpha identified by <password>;
...
alter user alpha default tablespace users;
User altered.
```
...
```
impdp alpha/<password>@pdb2 dumpfile=source_pdb1.dmp logfile=source_pdb1.log directory=tmp full=y encryption_password=<password>
...
```
[oracle@workshop1-2 ~]$ sqlplus alpha/<password>@pdb2
...
```
sudo su - oracle
[opc@workshop1-2 ~]$ sudo su - oracle
...
```
connect alpha/<password>@pdb2

create database link pdb1.sub01092328510.vcn1.oraclevcn.com
connect to alpha identified by <password>
using '(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=workshop1-scan)(PORT=1521))(CONNECT_DATA=(SERVER=DEDICATED)(SERVICE_NAME=pdb1.sub01092328510.vcn1.oraclevcn.com)))'; 

Database link created.

SQL> select sysdate from dual@pdb1.sub01092328510.vcn1.oraclevcn.com;
...
```
select table_name from user_tables@pdb1.sub01092328510.vcn1.oraclevcn.com;
...
```
create table order_items_copy as select * from order_items@pdb1.sub01092328510.vcn1.oraclevcn.com;
