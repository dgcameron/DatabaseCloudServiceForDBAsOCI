...
- Create a new ssh key pair.  Open a terminal window on your desktop and enter the following.  Name the key **privateKey**.  Do not give it a passphrase.  Depending on whether you are on Linux, MAC, or Windows the output may be different.
```
...
- Copy the container database entries in the tnsnames.ora file two times, and update as follows.  These entries provide connectivity to the pluggable databases.
