...
- Open a command window in the workshop directory and scp the **oci_ws_alpha.dmp** file to dbcs.  Enter the following command:
	- **scp -i keys/privateKey data/oci_ws_alpha.dmp opc@<dbcs IP>:/tmp**

	![](images/1/037.png)

- SSH to dbcs.  Enter the following command in a terminal window.
	- **ssh -i keys/privateKey opc@<dbcs IP>**

- Sudo to oracle user.
	- **sudo su - oracle**

- Update the tnsnames file to add pdb1, pdbsrc, and pdbtgt pluggable databases.  We will be creating these in the following steps.  Enter the following:
