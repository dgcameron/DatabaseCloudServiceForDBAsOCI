...
This is the second of five labs that are part of the Oracle Public Cloud Database Cloud Service workshop. 

Since labs 200 and 300 will be migrating data, it is important to understand and keep track of what you're doing.  Briefly these will be the steps.
- Lab 1 creates source (`workshop1_db`) and target (`workshop1_db2`) databases and loads data into the `alpha` schema into `workshop1_db`.  The target has an empty `pdb1`.
...
- Lab 3 remote clones `pdb1` from the source database to `pdb2` in the target.  The schema `alpha` will be in all pdbs.  Lab 3 also creates a new tablespace in the source and then migrates this to the target database.  It also migrates data at the schema level by exporting `alpha` schema from the source and imports this into a new `alpha_archive` schema in the target database.  Finally you receate a target copy of a table in the source pdb1 database.
- Labs 4 and 5 do not involve migration and can be done in either database. 
