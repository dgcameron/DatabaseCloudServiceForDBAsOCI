...
- Lab 3 creates a new tablespace `alpha_archive` and user `alpha_archive` in the source database and then migrates this to the target database.  It also migrates data by exporting the `alpha` schema from the source and importing this into a new `alpha_archive` schema in the target database using data pump .  Finally you receate a target copy of a table in the source pdb1 database.  These alternative migration approaches support different use cases that will be discussed in the lab steps.
