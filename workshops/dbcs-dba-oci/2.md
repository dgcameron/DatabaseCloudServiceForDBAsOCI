...
```
<copy>ssh -i id_rsa opc@129.146.162.124
sudo su - oracle
...
- SCP (copy) the private key to the workshop image.  id_rsa is entered twice - once to authenticate and again as the file to copy to the image.  This id_rsa private key will be used to connect the workshop image to the source and target databases when the migration is run.  
```
