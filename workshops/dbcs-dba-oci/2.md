...
- SCP (copy) the private key to the workshop image.  PrivateKey is entered twice - once to authenticate and again as the file to copy to the image.  This privateKey will be used to connect the workshop image to the source and target databases when the migration is run.  
```
<copy>scp -i privateKey privateKey opc@&lt;workshop image IP&gt;:/tmp</copy>
```
...
-  Enter the following.  You will be prompted for the sys database password and the userid auth token.  When the job kicks off it will return a job id, which you can then query to review the progress.  This will take a few minutes.  You should see `COMPLETED` status for all steps, otherwise you may have an error.  ***Be sure to replace values to reflect your environment (eg: sourcedb, backupuser, etc).***
```
<copy>zdmhome/bin/zdmcli migrate database -sourcedb orcl_phx273 -sourcenode workshop1-scan -srcauth zdmauth -srcarg1 user:opc -srcarg2 identity_file:/tmp/privateKey -srcarg3 sudo_location:/usr/bin/sudo -targetnode workshop1-2-scan -targethome /u01/app/oracle/product/19.0.0.0/dbhome_1 -backupuser oracleidentitycloudservice/derrick.cameron@oracle.com -rsp /u01/app/zdmhome/rhp/zdm/template/zdm_template.rsp -tgtauth zdmauth -tgtarg1 user:opc -tgtarg2 identity_file:/tmp/privateKey -tgtarg3 sudo_location:/usr/bin/sudo -ignore ALL -eval</copy>
...
Scheduled job command: "zdmcli migrate database -sourcedb orcl_phx273 -sourcenode workshop1-scan -srcauth zdmauth -srcarg1 user:opc -srcarg2 identity_file:/tmp/privateKey -srcarg3 sudo_location:/usr/bin/sudo -targetnode workshop1-2-scan -targethome /u01/app/oracle/product/19.0.0.0/dbhome_1 -backupuser oracleidentitycloudservice/derrick.cameron@oracle.com -rsp /u01/app/zdmhome/rhp/zdm/template/zdm_template.rsp -tgtauth zdmauth -tgtarg1 user:opc -tgtarg2 identity_file:/tmp/privateKey -tgtarg3 sudo_location:/usr/bin/sudo -ignore ALL -eval"
...
```
<copy>zdmhome/bin/zdmcli migrate database -sourcedb orcl_phx273 -sourcenode workshop1-scan -srcauth zdmauth -srcarg1 user:opc -srcarg2 identity_file:/tmp/privateKey -srcarg3 sudo_location:/usr/bin/sudo -targetnode workshop1-2-scan -targethome /u01/app/oracle/product/19.0.0.0/dbhome_1 -backupuser oracleidentitycloudservice/derrick.cameron@oracle.com -rsp /u01/app/zdmhome/rhp/zdm/template/zdm_template.rsp -tgtauth zdmauth -tgtarg1 user:opc -tgtarg2 identity_file:/tmp/privateKey -tgtarg3 sudo_location:/usr/bin/sudo -ignore ALL</copy>
...
connect alpha/&lt;password&gt;@pdb1;</copy>
